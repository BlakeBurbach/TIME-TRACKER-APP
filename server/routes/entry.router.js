...
router.post('/', (req, res) => {
    ...
    console.log('POST /entries route', entryToAdd);
    // add user's entry data to time_tracker database under entries
    let queryText = `INSERT INTO entries (description, start_date, end_date, projects_id)
                     VALUES ($1, $2, $3, $4);`
...
router.get('/', (req, res) => {
    ...
    const queryText = `SELECT entries.*, 
                       ...
                       entries.description as entry_desc,
                       ...
                       FROM entries JOIN projects ON entries.projects_id = projects.id;`;
